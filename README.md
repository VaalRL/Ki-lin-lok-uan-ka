...
- **點擊即全選與自動換格**：點擊欄位自動全選現有數值，按下 Enter 直接發動播放。
- **智慧跳格**：課次欄會依目前書籍的實際課次上限判斷 —— 若輸入的數字不可能是兩位數課次的開頭（例如 Action 有 22 課，輸入 3 以上），游標立即跳至曲目格；輸入 1 或 2 時則等待第二碼，避免第 10~22 課無法輸入。
...
專為在筆電、桌機或外接鍵盤 iPad 上自學聽讀設計，無需頻繁移動滑鼠即可流暢操控（打字時自動防誤觸）：
| 快捷鍵 | 功能說明 | 備註 |
| :--- | :--- | :--- |
| `空白鍵 (Space)` | 播放 / 暫停 (Play / Pause) | 帶頂部 HUD 狀態提示 |
| `←` / `→` | 快退 5 秒 / 快進 5 秒 | 會話句子精聽必備 |
| `[` / `]` | 上一首 / 下一首 | 跨冊自動銜接 |
| `↑` / `↓` | 提高 / 降低音量 (±5%) | 螢幕同步顯示音量百分比 |
| `M` | 靜音 / 恢復音量 (Mute) | 即時靜音切換 |
| `L` | 單曲循環開關 (Loop) | 一鍵鎖定單句練習 |
| `R` | 從頭重播本首 (Restart) | 即時回到 0:00 |
| `/` | 聚焦至曲目搜尋框 (Search) | 快速檢索課文曲目 |
| `Esc` | 離開輸入框焦點 | 恢復全局鍵盤操控 |

> 快捷鍵配置與姊妹專案 [河馬隨身聽](https://github.com/VaalRL/hippo-player) 一致，兩個 app 可共用同一套肌肉記憶。
...
本專案之開發與開源僅作為**個人家庭課後自主學習與無障礙介面優化之技術研究用途**。請詳閱完整的 [DISCLAIMER.md](DISCLAIMER.md)（[線上可讀版本](https://vaalrl.github.io/Ki-lin-lok-uan-ka/disclaimer.html)）：
...
本專案無任何分析工具、廣告或第三方追蹤程式碼，不蒐集也不傳送任何使用者資料。
唯一的對外連線是瀏覽器直接向遠端伺服器請求音訊與曲目清單；該伺服器會如同您直接開啟該網址一般取得 IP 位址，
